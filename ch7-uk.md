...
Теорема для функції `filter` схожа. Вона каже, що, якщо ми робимо композицію з `f` та `p`, щоб перевірити, яка має бути відфільтрованою, після цього застосовуємо `f` за допомогою `map`(пам'ятайте, фільтер не трансформуватиме елементи - його сигнатура чітко зазначає, що `a`не буде зачіпатись), то це буде завжди еквівалентно проходженню за допомогою `map` по нашій `f` і потім відфільтровування результату за допомогою предикату `p`. 
...
[Chapter 8: Tupperware](ch8-uk.md)
